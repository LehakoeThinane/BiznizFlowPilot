...
def create_event(
    ...
    try:
        ...
        event = service.create(current_user.business_id, current_user, data)
        db.commit()
        return event
...
def list_events(
    ...
    try:
        ...
        elif event_type and entity_type and entity_id:
...
@router.get("/audit-trail/{entity_type}/{entity_id}", response_model=dict)
def get_audit_trail(
    entity_type: str,
    entity_id: UUID,
    current_user: Annotated[CurrentUser, Depends(get_current_user)],
    ...
    try:
        service = EventService(db)
        return service.get_audit_trail(current_user.business_id, current_user, entity_type, entity_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
